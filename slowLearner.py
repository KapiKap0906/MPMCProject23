...
crsr = sqlite3.connect('Dataset.sqlite')
...
def calculateThreshold():
    updateAverages("TheoryMarks")
    updateAverages("LabMarks")
    c.execute("SELECT * FROM TheoryMarks WHERE name = 'Average'")
    theoryAverage = c.fetchone()
    theoryAverage = theoryAverage[1:]
    c.execute("SELECT * FROM LabMarks WHERE name = 'Average'")
    labAverage = c.fetchone()
    labAverage = labAverage[1:]
    '''
    averages = theoryAverage + labAverage
    meanAverage = statistics.mean(averages)
    '''
    Threshold = abs(statistics.mean(theoryAverage) - statistics.mean(labAverage))
    print("The THRESHOLD value has been set to ",Threshold)
